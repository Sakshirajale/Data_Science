# -*- coding: utf-8 -*-
"""
Created on Thu Sep 26 16:18:53 2024
...
import requests
link="https://www.amazon.in/Fujifilm-Instax-Happiness-Shots-Purple/product-reviews/B0C9ZQZ8PH/ref=cm_cr_dp_d_show_all_btm?ie=UTF8&reviewerType=all_reviews"
...
rate_cleaned1=[float(r.split(' ')[0]) for r in rate1]
rate_cleaned1
#############Review body############
review1=soup.find_all('span',class_='a-size-base review-text review-text-content')
review1
review_body1=[]
...
from bs4 import BeautifulSoup as bs
import requests
link="https://www.imdb.com/title/tt1187043/reviews/?ref_=tt_ov_ql_2"
page=requests.get(link)
page
page.content
## now let us parse the html page
...
import pandas as pd
df=pd.DataFrame()
df['review_title1']=review_titles
df['rate1']=rate
df['review_body1']=review_body
df
...
from bs4 import BeautifulSoup as bs
import requests
link="https://www.myntra.com/reviews/29239270"
page=requests.get(link)
page
page.content
## now let us parse the html page
...
print(soup.prettify())
#for title
title=soup.find_all('span',class_='a-size-base review-title a-text-bold')
title
